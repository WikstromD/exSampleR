...
When you choose a variable and generate plots, the app displays a **3×3** **grid of panels**. One panel contains the real data; the rest are simulated normal distributions (Shapiro Wilk *p* \< 0.01) with the same number of observations, mean, and standard deviation. Below is an illustrative example using the variable "Heavy_Right_Skew" from the example dataset included with `exsampler`.
